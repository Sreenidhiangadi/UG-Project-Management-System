...
function ProjectById() {
    ...
    //     setteamMemberCount(currentProject.team.length)
    //     setteammember(currentProject.team)
    //     setinresearch(currentProject.publicationDetails?true:false)
    //   }
    // })
    useEffect(() => {
      if (state) {
  ...
  }, [state]); // Ensure to include state in the dependency array if needed
    ...
    const modifiedProject=async(editedProject)=>{
      ...
      const transformedCategory = editedProject.category?.value || null;
        ...
        delete pro._id;
        let project={...pro,category:transformedCategory,
  ...
  return (
    <div className='container mt-5' style={{ height: '80vh', overflow: 'auto' }}>
        {projectEditStatus===false?(
          ...
          <div>  <h2>{currentProject.title}</h2>
          <button className='btn btn-primary' type='submit' onClick={disableEditStatus}>Edit</button>
          {currentProject.status===true?(
            <button type='submit' className='btn btn-primary' onClick={deleteProject}>Delete</button>):(
            <button type='submit' className='btn btn-primary' onClick={restoreProject}>Restore</button>)}
